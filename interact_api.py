...
class MainHandler(tornado.web.RequestHandler):
    ...
    args=''
    @classmethod
    def setValue(self, personality, history, tokenizer, model, args):
    ...
    def get(self):
        ...
        with torch.no_grad():
            out_ids = sample_sequence(personality, self.history, tokenizer, model, args)
        self.history.append(out_ids)
        history = self.history[-(2 * args.max_history + 1):]
        out_text = tokenizer.decode(out_ids, skip_special_tokens=True)
        print(history)
        print(out_text)
        self.write(out_text)
...
if __name__ == "__main__":
    ...
    history = []
    handle=MainHandler
    handle.setValue(personality, history, tokenizer, model, args)
    application = tornado.web.Application([(r"/nextSentence", handle), ])
